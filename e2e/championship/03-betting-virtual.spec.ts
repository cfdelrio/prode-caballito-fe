...
async function selectPlanilla(page: any) {
  ...
  const option = selector.getByText(PLANILLA_NAMES.virtual)
  if (await option.isVisible().catch(() => false)) {
    await option.click()
  } else {
    await selector.locator('select').selectOption({ label: PLANILLA_NAMES.virtual }).catch(async () => {
      await selector.click()
      await page.getByText(PLANILLA_NAMES.virtual).click()
    })
  }
}
